...
export async function POST(request:Request){
  ...
  try{
    ...
    if(action==="sign"){
      ...
      return NextResponse.json({objectKey,uploadUrl:await presignMvpPut(config,objectKey,{},GAME_MEDIA_SIGNING_SECONDS),publicUrl:gameMediaPublicUrl(config,objectKey),requiredHeaders:headers,expiresAt:new Date(Date.now()+GAME_MEDIA_SIGNING_SECONDS*1000).toISOString()});
    }
    if(action==="verify"){
