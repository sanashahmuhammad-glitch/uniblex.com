...
export async function GET(request:Request){
  ...
  const {data,error}=await createServiceSupabaseClient().from("game_submissions").select("id,title,slug,status,engine,updated_at,build_verified,short_description").eq("owner_id",auth.user.id).order("updated_at",{ascending:false}).limit(200);
...
export async function POST(request:Request){
  const auth=await verifyDeveloperRequest(request);if(!auth.authorized)return NextResponse.json({error:auth.error},{status:401});
  try{
    const body=await request.json() as Record<string,unknown>;const db=createServiceSupabaseClient();
