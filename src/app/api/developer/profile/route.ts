...
export async function GET(request: Request) {
  ...
  const {data,error}=await createUserSupabaseClient(request.headers.get("authorization") || "").from("developer_profiles").select("*").eq("id",auth.user.id).maybeSingle();
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const {data,error}=await createUserSupabaseClient(request.headers.get("authorization") || "").from("developer_profiles").upsert(payload,{onConflict:"id"}).select().single(); if(error)throw error;
