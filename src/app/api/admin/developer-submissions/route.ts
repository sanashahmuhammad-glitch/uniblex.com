...
export async function GET(request:Request){
  ...
  const db=createServiceSupabaseClient();const {data,error}=await db.from("game_submissions").select("*,developer_profiles!game_submissions_owner_id_fkey(studio_name,display_name,support_email),game_builds(id,verification_status,preview_url,build_type,compression_mode,file_count,total_bytes,verified_at),game_media(role,public_url),submission_reviews(id,decision,developer_feedback,checklist,created_at,reviewer_id)").order("updated_at",{ascending:false}).limit(200);
...
export async function POST(request:Request){
  ...
  try{
    ...
    const db=createServiceSupabaseClient();const {data:submission}=await db.from("game_submissions").select("id,owner_id,status,build_verified").eq("id",submissionId).maybeSingle();if(!submission)return NextResponse.json({error:"Submission was not found."},{status:404});if(["approved","published"].includes(decision)&&!submission.build_verified)return NextResponse.json({error:"An unverified build cannot be approved or published."},{status:409});
