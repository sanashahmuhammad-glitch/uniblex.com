...
export function SdkV2Docs() {
  return <section className="mx-auto max-w-6xl px-5 py-10 text-white"><h2 className="font-heading text-3xl">SDK v2 · optional host-managed advertising</h2><p className="mt-4 leading-7 text-uniblex-gray">Use one SDK version per page. The v1 download and lifecycle API remain unchanged. Version 2 keeps init() and game.loadingStart(), loadingStop(), ready(), gameplayStart(), and gameplayStop(). Games must start and remain playable when the SDK or host is absent.</p>
    ...
    <div className="mt-6 grid gap-5 md:grid-cols-2">{[
      ["Availability", "ads.isAvailable() returns a cached boolean; await ads.getCapabilities() refreshes interstitial and rewarded capability flags. Availability may change. No production provider is currently enabled."],
      ...
      ["Security and privacy", "The bridge validates source, origin, protocol version, session, and request ID. Games receive no account identity, private credentials, or revenue data. A local game can alter its own score; durable rewards and future earnings require server/provider verification."],
      ...
      ["Review and rendering", "Disclose providers, formats, external hosts, and ad destinations in the Developer Portal. Scans and manual review precede publication. The SDK is independent of WebGL/WebGPU; rendering fallback must be implemented and tested by each game build."],
