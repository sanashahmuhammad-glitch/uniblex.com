    ...
    <header className="sticky top-0 z-50 border-b border-white/10 bg-[#0D1118]/90 backdrop-blur-xl">
      <div className="mx-auto flex h-18 max-w-[1440px] items-center justify-between gap-6 px-4 py-3 sm:px-6 lg:px-8">
        ...
        <div className="hidden items-center gap-2 sm:flex">
          <Link href="/developers/login" className="btn-secondary !min-h-10 !px-4 !py-2 text-sm">Log in</Link>
          <Link href="/developers/register" className="btn-primary !min-h-10 !px-4 !py-2 text-sm">Create account</Link>
        </div>
      ...
      {open ? (
        <nav className="grid gap-1 border-t border-white/10 bg-[#0b0f16] p-4 sm:hidden" aria-label="Mobile developer resources">
          ...
          <div className="mt-2 grid grid-cols-2 gap-2">
            <Link href="/developers/login" className="btn-secondary !px-3 text-sm">Log in</Link>
            <Link href="/developers/register" className="btn-primary !px-3 text-sm">Create account</Link>
          </div>
        </nav>
      ) : null}
    </header>
