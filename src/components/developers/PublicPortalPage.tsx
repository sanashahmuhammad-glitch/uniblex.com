...
export function SdkContent() {
  const install = `<script src="https://www.uniblex.com/sdk/uniblex-sdk-v1.js"></script>`;
  const usage = `await window.UniblexSDK.init();\nwindow.UniblexSDK.game.loadingStart();\n// Load your game assets\nwindow.UniblexSDK.game.loadingStop();\nwindow.UniblexSDK.game.ready();\nwindow.UniblexSDK.game.gameplayStart();`;
  return <main className="mx-auto max-w-6xl px-5 py-16 sm:px-6 lg:px-8"><p className="text-sm font-bold uppercase tracking-[.22em] text-uniblex-blue">Integration · v1.0</p><h1 className="mt-3 font-heading text-4xl text-white sm:text-6xl">Uniblex browser SDK</h1><p className="mt-5 max-w-3xl text-lg leading-8 text-uniblex-gray">A small, dependency-free host bridge for loading and gameplay lifecycle signals. Your game must remain playable when the bridge is unavailable.</p><div className="mt-10 grid gap-5 lg:grid-cols-2"><section className="card p-6"><h2 className="font-heading text-xl text-white">Install</h2><pre className="mt-4 overflow-x-auto rounded-xl border border-white/10 bg-black/35 p-4 text-sm text-emerald-200"><code>{install}</code></pre><a href="/sdk/uniblex-sdk-v1.js" download className="btn-secondary mt-5 text-sm">Download SDK v1</a></section><section className="card p-6"><h2 className="font-heading text-xl text-white">Initialize and signal readiness</h2><pre className="mt-4 overflow-x-auto rounded-xl border border-white/10 bg-black/35 p-4 text-sm text-uniblex-blue"><code>{usage}</code></pre></section></div><div className="mt-6 grid gap-4 md:grid-cols-3">{[{title:"Lifecycle",text:"Signal loading, ready, gameplay start, and gameplay stop at the real boundaries in your game."},{title:"Privacy",text:"The SDK sends no credentials, user identity, advertising ID, or game data. Events remain host lifecycle signals."},{title:"Fallback",text:"Guard SDK calls in builds that run on other hosts. Uniblex review checks that the game still starts safely."}].map((item)=><section key={item.title} className="card p-6"><h2 className="font-heading text-xl text-white">{item.title}</h2><p className="mt-3 leading-7 text-uniblex-gray">{item.text}</p></section>)}</div></main>;
}
