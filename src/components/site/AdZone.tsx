type Props = {
  label: string;
  size: "leaderboard" | "rectangle" | "in-content" | "game-bottom";
};

export function AdZone({ label, size }: Props) {
  const classes = {
    leaderboard: "min-h-[90px] max-w-[728px]",
    rectangle: "min-h-[250px] max-w-[300px]",
    "in-content": "min-h-[120px] w-full",
    "game-bottom": "min-h-[120px] w-full"
  };

  return (
    <div className={`mx-auto flex ${classes[size]} items-center justify-center rounded-2xl border border-dashed border-uniblex-border bg-uniblex-card/45 p-4 text-center text-xs uppercase tracking-[.2em] text-uniblex-gray`}>
      {label} Ad Zone
    </div>
  );
}
