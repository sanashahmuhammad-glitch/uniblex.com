...
import { Clock3, Gamepad2, Star, Trophy, Users } from "lucide-react";
...
export function GameCard({ game }: { game: Game }) {
  return (
    <article className="group flex h-full min-h-0 flex-col overflow-hidden rounded-xl border border-white/10 bg-gradient-to-br from-white/[.075] via-uniblex-card/85 to-black/35 shadow-[0_18px_60px_rgba(0,0,0,.24)] backdrop-blur transition duration-300 hover:-translate-y-1 hover:border-uniblex-blue/55 hover:shadow-[0_24px_80px_rgba(0,178,255,.16)]">
      <Link href={`/games/${game.slug}`} className="relative block aspect-video w-full shrink-0 overflow-hidden bg-black/30 [&>div>div]:hidden" aria-label={`Open ${game.title}`}>
      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        <Link className="mt-5 inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-uniblex-blue to-uniblex-purple px-4 py-2.5 text-sm font-black text-white shadow-[0_14px_32px_rgba(0,178,255,.24)] transition hover:brightness-110" href={`/games/${game.slug}`}>
